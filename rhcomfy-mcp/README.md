...
| `RUNNINGHUB_LIVE_CASES` | не задана | Безопасный boolean-флаг настроенности live harness; `full` требуется для явных платных live-проверок. |
...
Live-harness намеренно не входит в обычную установку. Для него нужен отдельный gate, а некоторые проверки могут отправлять платные задачи. Запускайте их только после проверки workflow и понимания возможных расходов.
