...
import { DerivedMediaService } from "../execution/derivatives.js";
...
import {} from "../graph/operations.js";
import { RevisionStore } from "../graph/revisions.js";
import { createWorkflowSchema, editWorkflowSchema, exportWorkflowSchema, getWorkflowSchema, validateWorkflowSchema, } from "../graph/schemas.js";
...
import { buildExamplePayloadSchema, createToolHandlers, endpointSchema, estimatePriceSchema, integrationGuideSchema, searchModelsSchema, validatePayloadSchema, } from "./upstream/tools.js";
function readCatalogManifest(catalogDir) {
    try {
        return JSON.parse(readFileSync(resolve(catalogDir, "..", "upstream", "manifest.json"), "utf8"));
    }
    catch {
        return {};
    }
...
function resultUri(resultId) {
    return `runninghub://result/${encodeURIComponent(resultId)}`;
}
function resultResourceLink(result) {
    return {
        type: "resource_link",
...
function inlineImageContent(storage, result) {
    if (!result.mime.toLowerCase().startsWith("image/") || result.size_bytes > MAX_INLINE_IMAGE_BYTES)
        return undefined;
...
function derivedUri(derivedId) {
    return `runninghub://derived/${encodeURIComponent(derivedId)}`;
}
function derivedResourceLink(derived) {
    return {
        type: "resource_link",
...
function validateApprovalContinuation(storage, resultId, continuation) {
    const result = storage.getResultById(resultId);
    if (!result)
        throw new AppError("PROJECT_NOT_FOUND", `Result ${resultId} was not found.`, { recoverable: true });
    const sourceJob = storage.getJob(result.job_id);
    if (!sourceJob)
        throw new AppError("PROJECT_NOT_FOUND", `Job ${result.job_id} was not found.`, { recoverable: false });
    ...
    if (!sourcePlan || !targetPlan)
        throw new AppError("PROJECT_NOT_FOUND", "The review continuation references a missing execution plan.", { recoverable: true });
    ...
    if (!sourceWorkItem || !targetWorkItem)
        throw new AppError("PROJECT_NOT_FOUND", "The review continuation references a missing work item.", { recoverable: false });
...
function continuationIntentPayload(value) {
    return JSON.stringify(value);
}
function parseContinuationIntent(payload) {
    let value;
    try {
    ...
    if (!value || typeof value !== "object" || typeof value.review_event_id !== "string" || typeof value.plan_id !== "string" || typeof value.request_id !== "string") {
        throw new AppError("INVALID_CONFIGURATION", "Stored approval continuation intent is malformed.", { recoverable: false });
    }
    return value;
}
function readStoredResource(storage, uri, row) {
    const job = storage.getJob(row.job_id);
    if (!job)
        throw new AppError("PROJECT_NOT_FOUND", `Job ${row.job_id} was not found.`, { recoverable: false });
    ...
    if (!project?.canonical_root)
        throw new AppError("INVALID_CONFIGURATION", `Project ${plan.project_id} has no canonical root.`, { recoverable: false });
    const projectRoot = realpathSync(resolve(project.canonical_root));
    const candidate = resolve(projectRoot, row.relative_path);
    if (!inside(projectRoot, candidate) || !existsSync(candidate))
        throw new AppError("DOWNLOAD_FAILED", `Stored result ${row.id} is unavailable.`, { recoverable: true });
    const resultPath = realpathSync(candidate);
    if (!inside(projectRoot, resultPath))
        throw new AppError("DOWNLOAD_FAILED", `Stored result ${row.id} escapes the project root.`, { recoverable: false });
    const bytes = readFileSync(resultPath);
    if (bytes.length !== row.size_bytes || bytesSha256(bytes) !== row.content_hash)
        throw new AppError("DOWNLOAD_FAILED", `Stored result ${row.id} failed its integrity check.`, { recoverable: true });
    return { contents: [{ uri: uri.toString(), mimeType: row.mime, blob: bytes.toString("base64") }] };
}
function readResultResource(storage, uri, variables) {
    const resultId = variables.result_id;
    if (typeof resultId !== "string")
        throw new AppError("PROJECT_NOT_FOUND", "Result resource ID is invalid.", { recoverable: true });
    const result = storage.getResultById(resultId);
    if (!result)
        throw new AppError("PROJECT_NOT_FOUND", `Result ${resultId} was not found.`, { recoverable: true });
    return readStoredResource(storage, uri, result);
}
function readDerivedResource(storage, uri, variables) {
    const derivedId = variables.derived_id;
    if (typeof derivedId !== "string")
        throw new AppError("PROJECT_NOT_FOUND", "Derived resource ID is invalid.", { recoverable: true });
    const derived = storage.getDerivedResultById(derivedId);
    if (!derived)
        throw new AppError("PROJECT_NOT_FOUND", `Derived result ${derivedId} was not found.`, { recoverable: true });
    const result = storage.getResultById(derived.result_id);
    if (!result)
        throw new AppError("PROJECT_NOT_FOUND", `Original result ${derived.result_id} was not found.`, { recoverable: false });
...
function configuredWorkflowBackend(config, profileId) {
...
function executionPlanForInput(input, projects, revisions, storage) {
    ...
    for (const binding of input.asset_bindings) {
        ...
        if (!asset)
            throw new AppError("ASSET_MISSING", `Asset ${binding.asset_id} is not registered in project ${workItem.project_id}.`, { recoverable: true });
...
async function safeUpstream(operation) {
    try {
...
export function createServer(config, storage) {
    ...
    const server = new McpServer({ name: "runninghub-mcp", version: "0.1.0" }, { instructions: SERVER_INSTRUCTIONS });
    server.resource("result-output", new ResourceTemplate("runninghub://result/{result_id}", { list: undefined }), { description: "A validated local original result file." }, async (uri, variables) => readResultResource(storage, uri, variables));
    server.resource("derived-output", new ResourceTemplate("runninghub://derived/{derived_id}", { list: undefined }), { description: "A validated local derived preview or video poster." }, async (uri, variables) => readDerivedResource(storage, uri, variables));
    server.tool("rh_search_models", "Search the pinned public RunningHub model catalog. This is discovery only, not account availability.", searchModelsSchema, async (input) => safeUpstream(() => handlers.rh_search_models(input)));
    server.tool("rh_get_model_schema", "Return a public schema for one catalog endpoint; it does not verify cloud access.", endpointSchema, async (input) => safeUpstream(() => handlers.rh_get_model_schema(input)));
    server.tool("rh_estimate_price", "Estimate a public catalog price. Verify official RunningHub pricing before showing a final cost.", estimatePriceSchema, async (input) => safeUpstream(() => handlers.rh_estimate_price(input)));
    server.tool("rh_validate_payload", "Validate a payload against a public model schema; this does not execute it.", validatePayloadSchema, async (input) => safeUpstream(() => handlers.rh_validate_payload(input)));
    server.tool("rh_get_integration_guide", "Return public-safe integration guidance for the optional standard model API family.", integrationGuideSchema, async (input) => safeUpstream(() => handlers.rh_get_integration_guide(input)));
    server.tool("rh_build_example_payload", "Build an example payload from a public model schema; it does not submit a task.", buildExamplePayloadSchema, async (input) => safeUpstream(() => handlers.rh_build_example_payload(input)));
    server.tool("rh_create_workflow", "Create an immutable local workflow revision from an API graph or an empty draft. No cloud task is submitted.", createWorkflowSchema, async (input) => {
        try {
    ...
    server.tool("rh_get_workflow", "Read a local immutable workflow revision and its validation report.", getWorkflowSchema, async (input) => {
        try {
    ...
    server.tool("rh_edit_workflow", "Apply an atomic typed graph edit batch using compare-and-swap; failed batches create no revision.", editWorkflowSchema, async (input) => {
        try {
    ...
    server.tool("rh_validate_workflow", "Return structural, runnable, and backend-compatibility states for a local workflow revision.", validateWorkflowSchema, async (input) => {
        try {
    ...
    server.tool("rh_export_workflow", "Export a local workflow revision as API-format JSON without secrets or cloud submission.", exportWorkflowSchema, async (input) => {
        try {
    ...
    server.tool("rh_project", "Register, inspect, list, or index a project without changing files outside its explicit roots.", projectToolSchema, async (input) => {
        try {
            if (input.action === "list")
                return { content: [jsonText(okResult(projects.listProjects()))] };
            if (!input.project_id)
                throw new AppError("INVALID_CONFIGURATION", "project_id is required for this project action.", { recoverable: true });
            if (input.action === "inspect")
                return { content: [jsonText(okResult(projects.getProject(input.project_id)))] };
            if (input.action === "index")
                return { content: [jsonText(okResult(projects.indexProject(input.project_id)))] };
            if (!input.canonical_root || !input.backend_profile_id || !input.output_root)
                throw new AppError("INVALID_CONFIGURATION", "canonical_root, backend_profile_id, and output_root are required to register a project.", { recoverable: true });
            return {
    ...
    server.tool("rh_asset", "Inspect or register project-owned assets, prepare a tagged provider reference, or explicitly upload one through the configured backend.", assetToolSchema, async (input) => {
        try {
            ...
            if (input.action === "register") {
                if (!input.relative_path)
                    throw new AppError("INVALID_CONFIGURATION", "relative_path is required to register an asset.", { recoverable: true });
                return { content: [jsonText(okResult(projects.registerAsset(input.project_id, input.relative_path, input.roles ?? [])))] };
            }
            if (!input.asset_id)
                throw new AppError("INVALID_CONFIGURATION", "asset_id is required for this asset action.", { recoverable: true });
            const asset = assetProvider.inspect(input.project_id, input.asset_id);
            if (Array.isArray(asset))
                throw new Error("Expected one asset");
            ...
            if (!input.work_item_id)
                throw new AppError("INVALID_CONFIGURATION", "work_item_id is required for provider upload.", { recoverable: true });
            const workItem = projects.getWorkItem(input.work_item_id);
            if (workItem.project_id !== input.project_id)
                throw new AppError("ASSET_MISSING", "The work item does not belong to the asset project.", { recoverable: true });
    ...
    server.tool("rh_upload_lora", "Upload one registered project LoRA through the dedicated RHLoraLoader flow; signed upload URLs are never returned or stored as graph references.", uploadLoraSchema, async (input) => {
        try {
            const asset = assetProvider.inspect(input.project_id, input.asset_id);
            if (Array.isArray(asset))
                throw new Error("Expected one asset");
            const workItem = projects.getWorkItem(input.work_item_id);
            if (workItem.project_id !== input.project_id)
                throw new AppError("ASSET_MISSING", "The work item does not belong to the LoRA asset project.", { recoverable: true });
            if (workItem.state !== "REQUESTED")
                throw new AppError("INVALID_CONFIGURATION", `Work item ${workItem.id} is not open for LoRA upload.`, { recoverable: true });
            if (!isLoraAsset(asset))
                throw new AppError("INVALID_CONFIGURATION", `Asset ${asset.id} is not registered with the lora role.`, { recoverable: true });
            const profileId = input.backend_profile_id ?? projects.getProject(input.project_id).backend_profile_id;
            const backend = configuredWorkflowBackend(config, profileId);
    ...
    server.tool("rh_scene", "Read, resolve, or upsert normalized scene context and its required asset roles.", sceneToolSchema, async (input) => {
        try {
            if (input.action === "resolve") {
                if (!input.alias)
                    throw new AppError("INVALID_CONFIGURATION", "alias is required to resolve a scene.", { recoverable: true });
                return { content: [jsonText(okResult(projects.resolveScene(input.project_id, input.alias)))] };
            }
            if (!input.scene_id)
                throw new AppError("INVALID_CONFIGURATION", "scene_id is required for read/upsert.", { recoverable: true });
            if (input.action === "read")
                return { content: [jsonText(okResult(projects.getScene(input.project_id, input.scene_id)))] };
            if (!input.action_text || !input.output_kind || !input.constraints || !input.required_asset_roles || !input.dependencies)
                throw new AppError("INVALID_CONFIGURATION", "upsert requires action_text, output_kind, constraints, required_asset_roles, and dependencies.", { recoverable: true });
            return {
    ...
    server.tool("rh_work_item", "Create/read/close a requested work item; creation does not create a job or submit a paid task.", workItemToolSchema, async (input) => {
        try {
            if (input.action === "read" || input.action === "close") {
                if (!input.work_item_id)
                    throw new AppError("INVALID_CONFIGURATION", "work_item_id is required.", { recoverable: true });
            ...
            if (!input.project_id || !input.user_request || !input.request_kind)
                throw new AppError("INVALID_CONFIGURATION", "create requires project_id, user_request, and request_kind.", { recoverable: true });
    ...
    server.tool("rh_search_workflows", "Search the registered workflow library using hard scene requirements; constraints are never weakened to force a result.", workflowSearchSchema, async (input) => {
        try {
    ...
    server.tool("rh_import_workflow", "Discover API-format workflow JSON files in a registered project or import one explicitly; no provider task is submitted.", importWorkflowSchema, async (input) => {
        try {
            const candidates = projects.discoverWorkflowFiles(input.project_id);
            if (!input.relative_path) {
                if (candidates.length === 0)
                    throw new AppError("SCHEMA_UNKNOWN", `No API-format workflow JSON was found in project ${input.project_id}.`, { recoverable: true, suggestedFix: "Place an exported API-format workflow JSON inside the project or provide an explicit relative_path." });
                if (candidates.length > 1)
                    return { content: [jsonText(okResult({ candidates }, ["Multiple project workflows were found; choose one explicitly or narrow the project workflow folder."]))] };
            }
            ...
            try {
                ...
                if (current.graph_hash !== source.graph_hash)
                    throw new AppError("REVISION_CONFLICT", `Workflow source ${source.relative_path} changed; create a new revision explicitly before execution.`, { recoverable: true });
                revision = current;
            }
    ...
    server.tool("rh_prepare_generation", "Create an immutable local execution plan from one work item and one workflow revision; no provider task is submitted.", prepareGenerationSchema, async (input) => {
        try {
    ...
    server.tool("rh_run_workflow", "Submit one prepared execution plan through its configured durable backend; repeated request IDs never create a second local submit.", runWorkflowSchema, async (input) => {
        try {
            const row = storage.getExecutionPlan(input.plan_id);
            if (!row)
                throw new AppError("PROJECT_NOT_FOUND", `Execution plan ${input.plan_id} was not found.`, { recoverable: true });
    ...
    server.tool("rh_job", "Inspect or advance one durable job without hiding provider errors or retrying an uncertain submit.", jobSchema, async (input) => {
        try {
            let row = storage.getJob(input.job_id);
            if (!row)
                throw new AppError("PROJECT_NOT_FOUND", `Job ${input.job_id} was not found.`, { recoverable: true });
            ...
            if (input.action === "resume" && input.provider_task_id) {
                ...
                if (!row)
                    throw new AppError("PROJECT_NOT_FOUND", `Job ${input.job_id} disappeared during reconcile.`, { recoverable: false });
            }
            if (input.action === "cancel" && !row.provider_task_id) {
                ...
                if (row.execution_state === "SUBMITTING") {
                    ...
                    if (!unknown)
                        throw new AppError("PROJECT_NOT_FOUND", `Job ${input.job_id} disappeared.`, { recoverable: false });
            ...
            if (!planRow)
                throw new AppError("PROJECT_NOT_FOUND", `Execution plan ${row.plan_id} was not found.`, { recoverable: false });
    ...
    server.tool("rh_get_results", "Download confirmed provider outputs as validated local original files, render supported images inline in the current MCP chat, and create local preview/poster derivatives without submitting another task.", getResultsSchema, async (input) => {
        try {
    ...
    server.tool("rh_review_result", "Record one idempotent user review for a saved result; an explicit approved continuation may run one already prepared next plan.", reviewResultSchema, async (input) => {
        try {
            ...
            if (continuation)
                validateApprovalContinuation(storage, input.result_id, continuation);
            const review = reviewService.review({
                ...reviewInput,
                ...(input.revision_request ? {
                    revision_request: {
                        ...input.revision_request,
                        operations: input.revision_request.operations,
                    },
                } : {}),
            });
            if (!continuation)
                return { content: [jsonText(okResult(review))] };
            const intentPayload = {
            ...
            if (!continuationPlanRow)
                throw new AppError("PROJECT_NOT_FOUND", `Execution plan ${continuation.plan_id} was not found.`, { recoverable: true });
    ...
    server.tool("rh_get_capabilities", "Report local server, catalog provenance, and backend capability evidence without exposing secrets.", {
