...
function issue(code, severity, message, node_id, input_name) {
    return {
        code,
        severity,
        message,
...
function portTypeCompatible(sourceType, targetType) {
    if (!sourceType || sourceType === "any" || targetType === "any")
        return true;
    if (sourceType === targetType)
        return true;
    if (targetType === "number" && sourceType === "integer")
        return true;
    return false;
}
function validateInputValue(value, port, issues, nodeId, name) {
    if (!port)
        return;
    if (value.kind === "link" || value.kind === "asset")
        return;
    if (value.kind === "integer") {
        ...
        if (Number.isSafeInteger(numeric)) {
            if (port.min !== undefined && numeric < port.min)
                issues.push(issue("INPUT_RANGE", "error", `Value is below minimum ${port.min}.`, nodeId, name));
            if (port.max !== undefined && numeric > port.max)
                issues.push(issue("INPUT_RANGE", "error", `Value is above maximum ${port.max}.`, nodeId, name));
        }
        return;
    }
    ...
    if (typeof literal === "number") {
        if (port.min !== undefined && literal < port.min)
            issues.push(issue("INPUT_RANGE", "error", `Value is below minimum ${port.min}.`, nodeId, name));
        if (port.max !== undefined && literal > port.max)
            issues.push(issue("INPUT_RANGE", "error", `Value is above maximum ${port.max}.`, nodeId, name));
    }
}
export function validateGraph(graph, catalog) {
    ...
    const structuralErrors = issues.some((entry) => entry.severity === "error" &&
...
export function assertStructurallyValid(report) {
