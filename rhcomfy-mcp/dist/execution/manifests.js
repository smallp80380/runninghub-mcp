...
function parseNodeClasses(graphJson) {
    try {
        const parsed = parseLossless(graphJson);
...
function manifestFromBytes(bytes, manifestId) {
    let parsed;
    try {
    ...
    if (!parsed || typeof parsed !== "object" || parsed.manifest_id !== manifestId) {
...
export class ResultManifestService {
    ...
    create(jobId, apiFamily) {
        ...
        if (!existing)
            this.writeManifest(context, bytes);
        ...
        if (!published?.published_at)
            throw new AppError("DOWNLOAD_FAILED", `Manifest ${jobId} remains unpublished.`, { recoverable: true });
        return {
            manifest_id: jobId,
            relative_path: context.manifestRelativePath,
            content_hash: manifestHash,
    ...
    publish(row) {
        ...
        if (!inside(context.projectRoot, realPath))
            throw new AppError("DOWNLOAD_FAILED", `Manifest ${row.aggregate_id} escapes the project root.`, { recoverable: false });
        ...
        if (!stored?.published_at)
            throw new AppError("DOWNLOAD_FAILED", `Manifest outbox ${row.id} was not marked published.`, { recoverable: true });
        return {
    ...
    context(jobId) {
        ...
        if (!project?.canonical_root || !project.output_root)
            throw new AppError("INVALID_CONFIGURATION", `Project ${plan.project_id} has no configured project root/output root.`, { recoverable: false });
        const projectRoot = realpathSync(resolve(project.canonical_root));
        if (!/^[A-Za-z0-9._-]+$/.test(job.id))
            throw new AppError("INVALID_CONFIGURATION", `Job ${job.id} cannot be used as a manifest path.`, { recoverable: false });
        const metadataRoot = resolve(projectRoot, ".runninghub");
        mkdirSync(metadataRoot, { recursive: true });
        const realMetadataRoot = realpathSync(metadataRoot);
        if (!inside(projectRoot, realMetadataRoot))
            throw new AppError("INVALID_CONFIGURATION", "Manifest metadata root escapes the project root.", { recoverable: false });
        const runsRoot = join(realMetadataRoot, "runs");
        mkdirSync(runsRoot, { recursive: true });
        const realRunsRoot = realpathSync(runsRoot);
        if (!inside(projectRoot, realRunsRoot))
            throw new AppError("INVALID_CONFIGURATION", "Manifest runs root escapes the project root.", { recoverable: false });
        const runDirectory = join(realRunsRoot, job.id);
        mkdirSync(runDirectory, { recursive: true });
        const realRunDirectory = realpathSync(runDirectory);
        if (!inside(projectRoot, realRunDirectory))
            throw new AppError("INVALID_CONFIGURATION", "Manifest run directory escapes the project root.", { recoverable: false });
    ...
    buildManifest(context, apiFamily) {
        ...
        if (!plan || !workItem || !revision)
            throw new AppError("PROJECT_NOT_FOUND", `Manifest dependencies for job ${context.job.id} were not found.`, { recoverable: false });
        const results = this.storage.listResults(context.job.id);
        if (!results.length)
            throw new AppError("DOWNLOAD_FAILED", `Job ${context.job.id} has no downloaded outputs for its manifest.`, { recoverable: true });
        const derivedByResult = new Map(results.map((result) => [result.id, this.storage.listDerivedResults(result.id)]));
        const assetBindings = JSON.parse(plan.asset_bindings_json);
        const manifest = {
            schema_version: "1",
            ...
            request: {
                ...
                ...(workItem.scene_id ? { scene_id: workItem.scene_id } : {}),
                allowed_outputs: JSON.parse(workItem.allowed_outputs_json),
                output_contract: JSON.parse(plan.output_contract_json),
            },
