...
export class DerivedMediaService {
    ...
    async deriveOne(result, kind) {
        ...
        if (!validPng(bytes))
            throw new AppError("DOWNLOAD_FAILED", `Generated ${kind} is not a valid PNG.`, { recoverable: true });
        ...
        if (!inside(context.projectRoot, absolutePath))
            throw new AppError("INVALID_CONFIGURATION", "Derived result path escapes the project root.", { recoverable: false });
    ...
    resultContext(result) {
        ...
        if (!project?.canonical_root || !project.output_root)
            throw new AppError("INVALID_CONFIGURATION", `Project ${plan.project_id} has no configured output root.`, { recoverable: false });
        ...
        if (!inside(projectRoot, sourceCandidate) || !existsSync(sourceCandidate))
            throw new AppError("DOWNLOAD_FAILED", `Stored result ${result.id} is unavailable.`, { recoverable: true });
        const sourcePath = realpathSync(sourceCandidate);
        if (!inside(projectRoot, sourcePath))
            throw new AppError("DOWNLOAD_FAILED", `Stored result ${result.id} escapes the project root.`, { recoverable: false });
    ...
    async renderPng(sourcePath, kind) {
        const args = [
            "-hide_banner",
            "-loglevel",
            "error",
