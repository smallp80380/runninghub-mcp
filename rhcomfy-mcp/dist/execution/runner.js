...
export function planToRow(plan, createdAt = new Date().toISOString()) {
    return {
        id: plan.id,
...
export function planFromRow(row) {
    return {
        ...
        asset_bindings: JSON.parse(row.asset_bindings_json),
        ...
        ...(row.provider_submit_mode ? { provider_submit_mode: row.provider_submit_mode } : {}),
        workflow_state: row.workflow_state,
        output_contract: JSON.parse(row.output_contract_json),
        mode: row.mode,
    };
}
export function jobHandle(row) {
    return {
        id: row.id,
        plan_id: row.plan_id,
...
export class DurableWorkflowRunner {
    ...
    prepare(plan) {
        ...
        const assets = new Map(this.assets.inspect(plan.project_id).map((asset) => [asset.id, asset]));
        ...
        if (!existing)
            this.storage.saveExecutionPlan(planToRow(plan));
        return plan;
    }
    async run(plan, requestId) {
        this.prepare(plan);
        let row;
        try {
        ...
        if (row.execution_state !== "READY")
            return jobHandle(row);
        let resolved;
        try {
            resolved = await this.resolveProviderWorkflow(plan);
        }
        catch (error) {
            if (error instanceof AppError && error.code === "REMOTE_WORKFLOW_DOES_NOT_ACCEPT_IMAGE_OVERRIDES")
                this.storage.updateExecutionPlanWorkflowState(plan.id, "failed_validation");
            throw error;
        }
        ...
        if (state !== "ready")
            throw new AppError("INVALID_CONFIGURATION", "Workflow must be ready before paid launch.", { recoverable: true });
        const intent = `submit:${row.id}`;
        if (!this.storage.claimSubmit(row.id, intent)) {
            const current = this.storage.getJob(row.id);
            if (!current)
                throw new Error(`Job ${row.id} disappeared`);
        ...
        try {
            ...
            if (error && typeof error === "object" && "name" in error && error.name === "SubmitUnknownError") {
                this.storage.markSubmitUnknown(row.id);
                const current = this.storage.getJob(row.id);
                if (!current)
                    throw new AppError("SUBMIT_UNKNOWN", "Submit intent was recorded but the job row disappeared.", { recoverable: false });
                return jobHandle(current);
            }
        ...
        const current = this.storage.getJob(row.id);
        if (!current)
            throw new Error(`Job ${row.id} disappeared after submit`);
        return jobHandle(current);
    }
    async wait(jobId, timeoutMs = 30_000, pollIntervalMs = 250) {
        ...
        while (Date.now() <= deadline) {
            ...
            if (row.execution_state === "SUBMIT_UNKNOWN")
                return jobHandle(row);
            if (row.execution_state === "FAILED" || row.execution_state === "CANCELLED")
                return jobHandle(row);
            if (row.execution_state === "SUCCEEDED" && row.artifact_state === "READY")
                return jobHandle(row);
    ...
    async recover(jobId, timeoutMs = 30_000) {
        const row = this.requireJob(jobId);
    ...
    async resolveProviderWorkflow(plan) {
        let parsed;
        try {
        ...
        const assetRows = new Map(this.assets.inspect(plan.project_id).map((asset) => [asset.id, asset]));
        ...
        if (plan.provider_workflow_id && parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
            for (const [nodeId, nodeValue] of Object.entries(parsed)) {
                if (!nodeValue || typeof nodeValue !== "object" || Array.isArray(nodeValue))
                    continue;
                const node = nodeValue;
                if (node.class_type !== "LoadImage")
                    continue;
                const inputs = node.inputs && typeof node.inputs === "object" && !Array.isArray(node.inputs) ? node.inputs : undefined;
                ...
                if (!binding)
                    throw new AppError("ASSET_MISSING", `Workflow contains an unbound asset reference ${image}.`, { recoverable: true });
                const reference = references.get(binding.asset_id);
                if (!reference)
                    throw new AppError("ASSET_MISSING", `No provider reference was prepared for asset ${binding.asset_id}.`, { recoverable: true });
        ...
        const replace = (value) => {
            if (typeof value === "string" && value.startsWith("asset://")) {
                ...
                if (!binding)
                    throw new AppError("ASSET_MISSING", `Workflow contains an unbound asset reference ${value}.`, { recoverable: true });
                const reference = references.get(binding.asset_id);
                if (!reference)
                    throw new AppError("ASSET_MISSING", `No provider reference was prepared for asset ${binding.asset_id}.`, { recoverable: true });
        ...
        if (resolved === undefined)
            throw new AppError("INVALID_GRAPH", "Could not serialize the provider workflow snapshot.", { recoverable: false });
        ...
        if (submitMode === "v2_node_info")
            await this.preflightRemoteImageOverrides(plan, nodeInfoList);
        return { workflow_json: resolved, node_info_list: nodeInfoList, submit_mode: submitMode };
    }
    async preflightRemoteImageOverrides(plan, nodeInfoList) {
        if (!plan.provider_workflow_id || !nodeInfoList.length)
            return;
        ...
        for (const override of nodeInfoList) {
            ...
            const inputs = node && typeof node === "object" && !Array.isArray(node) ? node.inputs : undefined;
            if (!node || typeof node !== "object" || node.class_type !== "LoadImage" || !inputs || typeof inputs !== "object" || !Object.prototype.hasOwnProperty.call(inputs, override.fieldName)) {
                throw new AppError("REMOTE_WORKFLOW_DOES_NOT_ACCEPT_IMAGE_OVERRIDES", "REMOTE_WORKFLOW_DOES_NOT_ACCEPT_IMAGE_OVERRIDES", {
                    recoverable: true,
