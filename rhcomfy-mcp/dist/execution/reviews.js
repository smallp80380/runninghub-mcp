...
import { ProjectContextService } from "../projects/context.js";
...
function aggregateStatus(states) {
    if (states.some((state) => state.status === "PENDING_REVIEW"))
        return "PENDING_REVIEW";
    if (states.some((state) => state.status === "CHANGES_REQUESTED"))
        return "CHANGES_REQUESTED";
    if (states.some((state) => state.status === "REJECTED"))
        return "REJECTED";
    return "APPROVED";
}
function nextActions(decision) {
    if (decision === "CHANGES_REQUESTED")
        return ["Create a new work item and revision explicitly from this feedback; the reviewed result remains unchanged."];
    if (decision === "APPROVED")
        return ["Wait for an explicit user request before starting another generation."];
...
export class ReviewService {
    ...
    revisions;
    projects;
    constructor(storage, revisions, projects) {
    ...
    getJobSummary(jobId) {
        ...
        if (!results.length)
            throw new AppError("DOWNLOAD_FAILED", `Job ${jobId} has no saved results to review.`, { recoverable: true });
    ...
    createChangesRequestedRevision(event, result, request) {
        ...
        if (!sourceRevisionRow)
            throw new AppError("PROJECT_NOT_FOUND", `Workflow revision ${plan.graph_revision_id} was not found.`, { recoverable: false });
...
function inputFeedback(event, reason) {
