...
export function checkModelCompatibility(graph, modelId, models, nodeCatalog) {
...
export function assertModelCompatible(report) {
...
export function replaceModelBinding(graph, loaderNodeId, modelId, models) {
    const node = graph.nodes[loaderNodeId];
    if (!node)
        throw new AppError("INVALID_GRAPH", `Loader node ${loaderNodeId} does not exist.`, { recoverable: true });
    const model = models.get(modelId);
    if (!model)
        throw new AppError("MODEL_INCOMPATIBLE", `Model ${modelId} is not registered.`, { recoverable: true });
