...
type MediaKind = "reference" | "mask" | "video";
...
interface MediaInfo {
  ...
  readonly png_color_type?: number;
  readonly exif_orientation?: number;
}

interface MediaValidationInput {
  readonly workflow: unknown;
  readonly asset_bindings: readonly { readonly asset_id: string; readonly content_hash: string }[];
...
function roles(asset: AssetRow): Set<string> {
  try {
    const value: unknown = JSON.parse(asset.roles_json);
    return new Set(Array.isArray(value) ? value.filter((role): role is string => typeof role === "string").map((role) => role.trim().toLowerCase()) : []);
  } catch {
    return new Set();
  }
}

function mediaError(message: string, context: Record<string, string> = {}): never {
...
function mediaKind(representation: string, asset: AssetRow): MediaKind {
  const value = representation.toLowerCase();
  if (value === "mask" || value.endsWith("_mask") || value === "image_mask") return "mask";
  if (value === "video" || value === "motion_video") return "video";
  if (asset.mime.toLowerCase().startsWith("video/")) return "video";
  if (asset.mime.toLowerCase().startsWith("image/")) return "reference";
  mediaError(`Asset ${asset.id} has unsupported media MIME ${asset.mime}.`, { asset_id: asset.id });
}

function assetReference(value: unknown): { readonly asset_id: string; readonly representation: string } | undefined {
...
function collectReferences(value: unknown, nodeId: string, inputName: string, assets: ReadonlyMap<string, AssetRow>, output: MediaReference[]): void {
  const reference = assetReference(value);
  if (reference) {
    const asset = assets.get(reference.asset_id);
    if (!asset) throw new AppError("ASSET_MISSING", `Asset ${reference.asset_id} referenced by ${nodeId}.${inputName} is not registered.`, { recoverable: true });
    if (isLoraAsset(asset) && reference.representation.toLowerCase() === "lora") return;
    output.push({ ...reference, node_id: nodeId, input_name: inputName, kind: mediaKind(reference.representation, asset) });
    return;
  }
  if (Array.isArray(value)) {
    for (const item of value) collectReferences(item, nodeId, inputName, assets, output);
    return;
  }
  if (isObject(value)) {
    for (const item of Object.values(value)) collectReferences(item, nodeId, inputName, assets, output);
  }
}

function collectWorkflowReferences(workflow: unknown, assets: ReadonlyMap<string, AssetRow>): MediaReference[] {
  if (!isObject(workflow)) mediaError("Workflow media validation requires an API-format object graph.");
  const references: MediaReference[] = [];
  for (const [nodeId, rawNode] of Object.entries(workflow)) {
    if (!isObject(rawNode) || !isObject(rawNode.inputs)) continue;
    for (const [inputName, value] of Object.entries(rawNode.inputs)) collectReferences(value, nodeId, inputName, assets, references);
  }
  return references;
}
...
function pngHeader(bytes: Uint8Array): PngHeader | undefined {
  ...
  if (bytes.length < signature.length || !signature.every((value, index) => bytes[index] === value)) return undefined;
  ...
  while (offset + 12 <= bytes.length) {
    ...
    if (type === "IHDR") {
      ...
      if (!width || !height || ![0, 2, 3, 4, 6].includes(colorType)) return undefined;
...
function jpegInfo(bytes: Uint8Array): Omit<MediaInfo, "kind" | "mime"> | undefined {
...
function inspectMedia(bytes: Uint8Array): MediaInfo | undefined {
  const png = pngHeader(bytes);
  if (png) return { kind: "image", mime: "image/png", has_alpha: png.has_alpha, png_color_type: png.color_type };
  const jpeg = jpegInfo(bytes);
  if (jpeg) return { kind: "image", mime: "image/jpeg", ...jpeg };
...
function validateRoleSemantics(reference: MediaReference, asset: AssetRow): void {
...
function validateBytes(reference: MediaReference, asset: AssetRow, bytes: Uint8Array): void {
  const info = inspectMedia(bytes);
  if (!info) mediaError(`Asset ${asset.id} is not a supported image or video container.`, { asset_id: asset.id });
...
export function validateMediaBindings(input: MediaValidationInput): { readonly profile_id: string; readonly references: number; readonly masks: number; readonly videos: number } {
  ...
  for (const reference of references) {
    ...
    if (input.strict_bytes) {
      ...
      if (!binding || !input.read_asset) throw new AppError("ASSET_MISSING", `Media asset ${reference.asset_id} has no readable execution binding.`, { recoverable: true });
