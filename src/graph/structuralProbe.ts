...
export interface ResizeProbeResult {
  ...
  readonly node_id: string;
  readonly before: { readonly width: string; readonly height: string };
  readonly after: { readonly width: string; readonly height: string };
}

function requestedDimension(value: number, name: string): string {
...
function existingDimension(graph: Graph, nodeId: string, inputName: "width" | "height"): string {
  ...
  if (value.kind === "integer") return value.decimal;
  ...
  if (value.kind === "link") return `link:${value.node_id}:${value.output_index}`;
...
export function prepareResizeGraph(graph: Graph, options: ResizeProbeOptions): ResizeProbeResult {
