...
interface UiNode {
  id: string | number;
  type: string;
  inputs?: Array<{ name: string; link?: string | number | null; type?: string }>;
...
function safeInteger(value: unknown): number | undefined {
  ...
  if (text === undefined || !/^(?:0|[1-9]\d*)$/.test(text)) return undefined;
  const parsed = Number(text);
  return Number.isSafeInteger(parsed) ? parsed : undefined;
}

function literal(value: unknown, path: string): JsonValue {
  ...
  if (Array.isArray(value)) return value.map((item, index) => literal(item, `${path}[${index}]`));
  if (value && typeof value === "object") return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, literal(item, `${path}.${key}`)]));
  throw new Error(`Unsupported UI literal at ${path}`);
}

function widgetValue(value: unknown, valueType: "string" | "number" | "integer" | "boolean", path: string): InputValue {
  if (valueType === "string") {
    if (typeof value !== "string") throw new Error(`Widget ${path} must be a string`);
    return { kind: "literal", value };
  }
  if (valueType === "boolean") {
    if (typeof value !== "boolean") throw new Error(`Widget ${path} must be boolean`);
    return { kind: "literal", value };
  }
  if (valueType === "integer") {
    const parsed = safeInteger(value);
    if (parsed !== undefined) return { kind: "integer", decimal: String(parsed) };
    if (typeof value === "string" && /^-?(?:0|[1-9]\d*)$/.test(value)) return { kind: "integer", decimal: value };
    throw new Error(`Widget ${path} must be a safe integer or canonical integer string`);
  }
  if (typeof value !== "number" || !Number.isFinite(value)) throw new Error(`Widget ${path} must be a finite number`);
  return { kind: "literal", value };
}

function rejectUnknownKeys(value: Record<string, unknown>, allowed: ReadonlySet<string>, context: string, source: string): void {
  const unknown = Object.keys(value).filter((key) => !allowed.has(key));
  if (unknown.length) throw new UiCodecError(`Unsupported UI extension at ${context}: ${unknown.join(", ")}.`, source);
}

export function importUiGraph(text: string, profile: UiNodeProfile): { graph: Graph; source_json: string; warnings: readonly string[] } {
  let raw: unknown;
  try {
  ...
  if (!Array.isArray(root.nodes) || !Array.isArray(root.links)) throw new UiCodecError("Supported UI JSON requires nodes and links arrays.", text);
  const linkMap = new Map<string, { source: string; output: number; target: string; input: number; type: string }>();
  for (const rawLink of root.links) {
    if (!Array.isArray(rawLink) || rawLink.length < 5) throw new UiCodecError("Unsupported UI link tuple.", text);
    const [link, source, output, target, input, type] = rawLink;
    const outputIndex = safeInteger(output);
    const inputIndex = safeInteger(input);
    if (outputIndex === undefined || inputIndex === undefined) throw new UiCodecError("UI link ports must be integers.", text);
  ...
  const uiPositions: Record<string, JsonValue> = {};
  for (const rawNode of root.nodes) {
    const value = object(rawNode) as UiNode;
    ...
    for (const rawInput of value.inputs ?? []) {
      const input = object(rawInput) as { name?: unknown; link?: unknown };
      if (typeof input.name !== "string") throw new UiCodecError(`UI input name missing at node ${id}.`, text);
      if (input.link !== undefined && input.link !== null) {
        const link = linkMap.get(String(input.link));
        if (!link || link.target !== id) throw new UiCodecError(`UI input ${id}.${input.name} points to an invalid link.`, text);
        inputs[input.name] = { kind: "link", node_id: link.source, output_index: link.output };
      }
    }
    const widgets = value.widgets_values ?? [];
    if (!Array.isArray(widgets)) throw new UiCodecError(`widgets_values must be an array at node ${id}.`, text);
    ...
    if (profile.output_node_types.includes(value.type)) outputNodes.push(id);
    if (Array.isArray(value.pos)) uiPositions[id] = literal(value.pos, `${id}.pos`);
  }
  return {
    graph: {
      schema_version: "1",
...
export function exportUiGraph(graph: Graph, profile: UiNodeProfile): string {
  ...
  for (const id of Object.keys(graph.nodes).sort()) {
    ...
    if (!node || !profile.supported_node_types.includes(node.class_type)) throw new UiCodecError(`Node ${node?.class_type ?? "unknown"} is not supported by UI profile.`, JSON.stringify(graph));
    const uiInputs: Array<Record<string, unknown>> = [];
    ...
    for (const [name, value] of Object.entries(node.inputs)) {
      ...
      if (value.kind === "link") {
        ...
        if (!Number.isSafeInteger(widgets[binding.widget_index])) throw new UiCodecError(`Integer widget ${id}.${name} cannot be represented safely in UI JSON.`, JSON.stringify(graph));
